...
function writeGeneratedArticle(targetPath, content) {
  writeFile(targetPath, content);
  writeFile(path.join(path.dirname(targetPath), GENERATED_MARKER), "Generated by scripts/build-news.mjs\n");
}
...
function cleanStaleArticleDirs(perLocale) {
  for (const locale of LOCALES) {
    const newsDir = path.join(SITE_ROOT, locale, "news");
    if (!fs.existsSync(newsDir)) continue;

    const expectedSlugs = new Set(perLocale[locale].map((article) => article.meta.slug));
    for (const entry of fs.readdirSync(newsDir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      if (expectedSlugs.has(entry.name)) continue;

      const targetPath = path.join(newsDir, entry.name);
      const indexPath = path.join(targetPath, "index.html");
      const markerPath = path.join(targetPath, GENERATED_MARKER);
...
function getArticleImage(article) {
  return article.meta.coverImage || `${SITE_URL}/artiou-og.png`;
}

function renderArticleCover(article) {
  if (!article.meta.coverImage) return "";
  const alt = article.meta.coverAlt || article.meta.title;
  return `        <figure class="article-cover">
          <img src="${escapeHtml(article.meta.coverImage)}" alt="${escapeHtml(alt)}" loading="eager" decoding="async" />
        </figure>`;
}

function buildArticleSchema(article, locale) {
  return {
...
function buildEventJsonLd(article, locale) {
  if (!article.meta.eventStartDate) return null;
  const event = {
    "@context": "https://schema.org",
    "@type": "Event",
    ...
    description: article.meta.description,
    inLanguage: LOCALE_UI[locale].htmlLang,
    startDate: article.meta.eventStartDate,
    eventStatus: "https://schema.org/EventScheduled",
    eventAttendanceMode: "https://schema.org/OfflineEventAttendanceMode",
    image: [getArticleImage(article)],
    url: `${SITE_URL}/${locale}/news/${article.meta.slug}/`,
    organizer: {
      "@type": "Organization",
      name: article.meta.eventLocationName || "Artiou",
    },
  };
  if (article.meta.eventEndDate) event.endDate = article.meta.eventEndDate;
  if (article.meta.eventLocationName) {
    event.location = {
      "@type": "Place",
      name: article.meta.eventLocationName,
      address: {
        "@type": "PostalAddress",
        streetAddress: article.meta.eventStreetAddress || "",
        addressLocality: article.meta.eventAddressLocality || "",
        postalCode: article.meta.eventPostalCode || "",
        addressCountry: article.meta.eventAddressCountry || "FR",
      },
    };
  }
  if (article.meta.eventPrice != null && article.meta.eventCurrency) {
    event.offers = {
      "@type": "Offer",
      price: article.meta.eventPrice,
      priceCurrency: article.meta.eventCurrency,
      availability: "https://schema.org/InStock",
      url: article.meta.sourceUrl || `${SITE_URL}/${locale}/news/${article.meta.slug}/`,
    };
  }
  return event;
}
...
function buildArticleJsonLd(article, locale) {
  const schemas = [buildArticleSchema(article, locale)];
  const eventSchema = buildEventJsonLd(article, locale);
...
function main() {
  ...
  cleanStaleArticleDirs(perLocale);
  ...
  for (const locale of LOCALES) {
    ...
    for (const article of articles) {
      const articleHtml = renderTemplate(articleTemplate, {
        ...
        OG_LOCALE: ui.ogLocale,
        ARTICLE_OG_IMAGE: escapeHtml(getArticleImage(article)),
        ...
        ARTICLE_COVER_HTML: renderArticleCover(article),
      ...
      writeGeneratedArticle(path.join(SITE_ROOT, locale, "news", article.meta.slug, "index.html"), articleHtml);
    }
  }
