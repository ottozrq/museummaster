...
def subFactoryGet(fixture, **kwargs):
    """
    To be used in fixture definition (or in the kwargs of the fixture constructor) to
    reference a other fixture using the :meth:`.BaseFix.get` method.

    :param fixture: Desired fixture :param kwargs: *Optional:* key words to overwrite
    properties of this fixture :return: Proxy object for the desired fixture including
    the altered properties
    """
...
def subFactoryCreate(fixture, **kwargs):
    """
    To be used in fixture definition (or in the kwargs of the fixture constructor) to
    reference a other fixture using the :meth:`.BaseFix.create` method.
...
def subFactoryModel(fixture, **kwargs):
    """
    To be used in fixture definition (or in the kwargs of the fixture constructor) to
    reference a other fixture using the :meth:`.BaseFix.model` method.
...
class BaseFix:
                ...
                try:
                    if False in [isinstance(a, SubFactory) for a in attr]:
                        raise AttributeError(
                            'References in fixtures must be declared w "SubFactory": '
                            + rel.key
                        )
                except TypeError:
    ...
    def model(self):
        """
        Returns a model instance of this fixture which is ready to be added. The model
        itself is not added to the DB but all dependencies are.

        :return: SQLAlchemy Model instance
        """
        # INFOs attr + relations: [f.key for f in Group._sa_class_manager.attributes]
        #
        # Attributes [(a.key, getattr(self, a.key)) for a in
        # self.MODEL._sa_class_manager.mapper.column_attrs]
        #
        # relations [a.key for a in Group._sa_class_manager.mapper.relationships]
    ...
    def create(self):
        """
        Adds this model to the session. This instance is not registered and thus can
        never be referred to museum get
        ...
        # in order to have the right values for all fields updated directly by the DB,
        # we have to load the model again
        return (
            self._fix_fact.get_db_session()
            .query(self.MODEL)
            .get(
                tuple(
                    getattr(model, field.name)
                    for field in class_mapper(self.MODEL).primary_key
                )
            )
        )
    ...
    def get(self):
        """
        returns an already existing model instance or creates one, registers it to be
        able to find it later and then returns the instance
    ...
    def getAttributes(self):
        ...
        attrs = dict(
            [
                (a.key, getAttr(a.key))
                for a in self.MODEL._sa_class_manager.mapper.attrs
        ...
        for hyb in inspect(self.MODEL).all_orm_descriptors.keys():
            if (
                inspect(self.MODEL).all_orm_descriptors[hyb].extension_type
                is hybrid.HYBRID_PROPERTY
            ):
